...
class TestJSONParser(unittest.TestCase):
    def test_gcal_event_kernel(self):
        ...
        mobilizon_metadata = first_group.event_template.publisher_specific_info[
            "mobilizon"
        ]
        ...
        self.assertEqual(
            "8qf27rmmeun7mat412odluuha11umbhm@import.calendar.google.com",
            first_group.calendar_ids[0],
        )
    ...
    def test_farmers_event_kernel(self):
        ...
        self.assertEqual(
            "e6d64dd3-e4c6-4390-88ff-7de4f34be821", mobilizon_metadata["defaultImageID"]
        )
    ...
    def test_runner_submission_json(self):
        ...
        self.assertIsInstance(
            mobilizon_publisher, MobilizonUploader, "Expected Mobilizon Uploader"
        )
        self.assertIsInstance(
            runner_submission.respective_scrapers[ScraperTypes.GOOGLE_CAL],
            GoogleCalendarScraper,
            "Expected Calendar Scraper",
        )
        self.assertIsInstance(
            runner_submission.respective_scrapers[ScraperTypes.STATIC],
            StaticScraper,
            "Expected Static Scraper",
        )

        static = group_packages[2].scraper_type_and_kernels[ScraperTypes.STATIC]
        for i in range(len(static)):
            self.assertEqual(
                farmers_market.scraper_type_and_kernels[ScraperTypes.STATIC][i],
                static[i],
            )
