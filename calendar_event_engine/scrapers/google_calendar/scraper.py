...
class GoogleCalendarScraper(Scraper):
    ...
    def _get_specific_calendar_events(
        self, google_calendar_id, group_kernel: GroupEventsKernel
    ):
        last_uploaded_event_date = None
        if not self.cache_db.no_entries_with_source_id(google_calendar_id):
            last_uploaded_event_date = self.cache_db.get_last_event_date_for_source_id(
                google_calendar_id
            )

        events: list[GenericEvent] = self.google_calendar_api.getAllEventsAWeekFromNow(
            calendarId=google_calendar_id,
            eventKernel=group_kernel.event_template,
    ...
    def retrieve_from_source(
        ...
        for google_calendar_id in group_event_kernel.calendar_ids:
            events = self._get_specific_calendar_events(
                google_calendar_id, group_event_kernel
            )
            all_events.append(
                AllEventsFromAGroup(events, group_event_kernel, google_calendar_id)
            )
    ...
    def connect_to_source(self):
        ...
        if use_oidc:
            token_path = (
                f"{os.getcwd()}/config/token.json"
                if "GOOGLE_API_TOKEN_PATH" not in os.environ
                else os.environ.get("GOOGLE_API_TOKEN_PATH")
            )
