...
class MobilizonUploader(Publisher):
    ...
    def create_cachable_response(
        ...
        upload_row = UploadedEventRow(
            uuid=upload_response["uuid"],
            user_id=upload_response["id"],
            title=event.title,
            date=event.begins_on,
            group_id=upload_response["groupId"],
            group_name=event_kernel.group_name,
        )
    ...
    def generic_event_converter(self, generic_event: GenericEvent):
        ...
        category = (
            None
            if "defaultCategory" not in mobilizon_metadata
        ...
        if validators.url(generic_event.picture):
            ...
            mobilizon_picture = (
                None
                if "defaultImageID" not in mobilizon_metadata
                else EventParameters.MediaInput(mobilizon_metadata["defaultImageID"])
            )

        mobilizon_tags = (
            None
            if "defaultTags" not in mobilizon_metadata
            else mobilizon_metadata["defaultTags"]
        )
        ...
        mobilizon_event = MobilizonEvent(
            mobilizon_metadata["groupID"],
            generic_event.title,
            description=generic_event.description,
            beginsOn=generic_event.begins_on,
            endsOn=generic_event.ends_on,
            tags=mobilizon_tags,
            onlineAddress=generic_event.online_address,
            physicalAddress=mobilizon_physical_address,
            category=category,
            picture=mobilizon_picture,
        )
