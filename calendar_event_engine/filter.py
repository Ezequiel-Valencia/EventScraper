import requests
import validators

from calendar_event_engine.types.generics import GenericEvent
from calendar_event_engine.types.submission import AllEventsFromAGroup


# https://www.geeksforgeeks.org/http-headers-content-type/


def normalize_generic_event(events_to_upload_list: list[AllEventsFromAGroup]):
    allowed_image_content_type_header = ["image/gif", "image/jpeg", "image/png"]
    for events_to_upload in events_to_upload_list:
        if events_to_upload.events is None:
            continue
        generic_event: GenericEvent
        for generic_event in events_to_upload.events:
            # If valid URL but not a picture, set to empty string
            if generic_event.picture is not None and validators.url(
                generic_event.picture
            ):
                picture_head = requests.head(generic_event.picture, timeout=30)
                if (
                    picture_head.status_code != 200
                    or picture_head.headers.get("Content-type")
                    not in allowed_image_content_type_header
                ):
                    generic_event.picture = ""

            if not validators.url(generic_event.online_address):
                generic_event.online_address = ""
